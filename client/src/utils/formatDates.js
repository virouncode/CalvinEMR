...
export const toLocalDateAndTime = (isoString, hour12 = true) => {
  ...
  const newDate = date.toISOString().slice(0, 10);
  const hours = toLocalHours(isoString, hour12);
  const minutes = toLocalMinutes(isoString);
  const ampm = toLocalAMPM(isoString);
...
export const toLocalDateAndTimeWithSeconds = (isoString, hour12 = true) => {
  ...
  const hours = toLocalHours(isoString, hour12);
  const minutes = toLocalMinutes(isoString);
  const seconds = toLocalSeconds(isoString);
  const ampm = toLocalAMPM(isoString);
...
export const toLocalMinutes = (isoString) => {
  if (isoString === "") return "";
  const newTime = toLocalTime(isoString);
